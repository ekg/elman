...
model = LadderLM(vocab_size=256, dim=1440, depth=3, level=9,
                 expansion=2.0, n_banks=4, core_ratio=0.5).cuda().bfloat16()
print(f'e9: dim=1440 depth=3 k=4 batch=192 params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * 192 * 512
print(f'DONE: e9 | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
