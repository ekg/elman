...
model = LadderLM(vocab_size=256, dim=256, depth=6, level=10,
                 expansion=1.0, n_banks=256, core_ratio=0.5).cuda().bfloat16()
print(f'e10_k256: dim=256 depth=6 k=256 batch=32 params={model.get_num_params():,}', flush=True)
...
buf = np.zeros((32, 513), dtype=np.uint8)
losses = []; start = time.time(); step = 0; peak_mem = 0

while time.time() - start < 600:
    step += 1
    pos = np.random.randint(0, data_len - 513, size=32)
