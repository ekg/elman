...
model = LadderLM(vocab_size=256, dim=352, depth=6, level=10,
                 expansion=1.0, n_banks=64, core_ratio=0.5).cuda().bfloat16()
print(f'e10_k64: dim=352 depth=6 k=64 batch=64 params={model.get_num_params():,}', flush=True)
...
buf = np.zeros((64, 513), dtype=np.uint8)
losses = []; start = time.time(); step = 0; peak_mem = 0

while time.time() - start < 600:
    step += 1
    pos = np.random.randint(0, data_len - 513, size=64)
