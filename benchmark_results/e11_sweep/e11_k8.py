...
model = LadderLM(vocab_size=256, dim=1184, depth=6, level=11,
                 expansion=1.0, n_banks=8, core_ratio=0.5).cuda().bfloat16()
print(f'e11_k8: dim=1184 depth=6 k=8 batch=224 params={model.get_num_params():,}', flush=True)
...
buf = np.zeros((224, 513), dtype=np.uint8)
losses = []; start = time.time(); step = 0; peak_mem = 0

while time.time() - start < 600:
    step += 1
    pos = np.random.randint(0, data_len - 513, size=224)
...
elapsed = time.time() - start
tokens = step * 224 * 512
print(f'DONE: e11_k8 | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
