...
model = LadderLM(vocab_size=256, dim=1184, depth=6, level=10,
                 expansion=1.0, n_banks=1, core_ratio=0.5).cuda().bfloat16()
batch_size = 320
print(f'e10_k1: dim=1184 depth=6 k=1 batch={batch_size} params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e10_k1 | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
