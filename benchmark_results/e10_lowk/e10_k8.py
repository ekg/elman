...
model = LadderLM(vocab_size=256, dim=800, depth=6, level=10,
                 expansion=1.0, n_banks=8, core_ratio=0.5).cuda().bfloat16()
print(f'e10_k8: dim=800 depth=6 k=8 batch=160 params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * 160 * 512
print(f'DONE: e10_k8 | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
