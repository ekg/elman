...
model = LadderLM(vocab_size=256, dim=2560, depth=6, level=1).cuda().bfloat16()
batch_size = 256
print(f'e1_200m: dim=2560 depth=6 batch={batch_size} params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e1_200m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
