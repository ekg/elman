...
model = LadderLM(vocab_size=256, dim=960, depth=6, level=10, n_banks=4).cuda().bfloat16()
batch_size = 288
print(f'e10_50m: dim=960 depth=6 k=4 batch={batch_size} params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e10_50m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
