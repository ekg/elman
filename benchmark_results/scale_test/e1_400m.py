...
model = LadderLM(vocab_size=256, dim=2304, depth=12, level=1).cuda().bfloat16()
print(f'e1_400m: params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e1_400m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s', flush=True)
mm.close()
