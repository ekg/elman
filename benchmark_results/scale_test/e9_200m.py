...
model = LadderLM(vocab_size=256, dim=2048, depth=6, level=9, expansion=2.0, core_ratio=0.5).cuda().bfloat16()
print(f'e9_200m: params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e9_200m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s', flush=True)
mm.close()
