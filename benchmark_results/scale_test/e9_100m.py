...
batch_size, time_limit = 192, 600

model = LadderLM(vocab_size=256, dim=1440, depth=6, level=9, expansion=2.0, core_ratio=0.5).cuda().bfloat16()
print(f'e9_100m: params={model.get_num_params():,}', flush=True)
...
while time.time() - start < time_limit:
    ...
    if step % 100 == 0:
        ...
        print(f'Step {step} | {elapsed:.0f}s | Loss {loss.item():.4f} | Avg100 {np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s', flush=True)

elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e9_100m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s', flush=True)
mm.close()
