...
batch_size, time_limit = 256, 600

model = LadderLM(vocab_size=256, dim=1440, depth=3, level=9, expansion=2.0, core_ratio=0.5).cuda().bfloat16()
print(f'e9_50m: params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e9_50m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s', flush=True)
mm.close()
