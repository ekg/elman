...
model = create_mamba2_model(target_params='100m', vocab_size=256).cuda().bfloat16()
print(f'mamba2_100m: params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: mamba2_100m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s', flush=True)
mm.close()
