...
model = LadderLM(vocab_size=256, dim=2048, depth=12, level=9, expansion=2.0, core_ratio=0.5).cuda().bfloat16()
layer = model.layers[0]
print(f'e9_400m_fixed: core={layer.core_dim} mem={layer.mem_dim} params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e9_400m_fixed | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
