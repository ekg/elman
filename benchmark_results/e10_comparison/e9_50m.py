...
model = LadderLM(
    vocab_size=256, dim=832, depth=9, level=9,
    expansion=2.0, n_banks=4, core_ratio=0.5
).cuda().bfloat16()

layer = model.layers[0]
if hasattr(layer, 'n_banks'):
    print(f'e9_50m: dim={model.dim} depth={model.depth} n_banks={layer.n_banks} params={model.get_num_params():,}', flush=True)
elif hasattr(layer, 'core_dim'):
    print(f'e9_50m: dim={model.dim} depth={model.depth} core={layer.core_dim} mem={layer.mem_dim} params={model.get_num_params():,}', flush=True)
else:
    print(f'e9_50m: dim={model.dim} depth={model.depth} params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e9_50m | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
