...
model = LadderLM(
    ...
    expansion=1.0, n_banks=256, core_ratio=0.5
).cuda().bfloat16()

layer = model.layers[0]
if hasattr(layer, 'n_banks'):
    print(f'e10_k256: dim={model.dim} depth={model.depth} n_banks={layer.n_banks} params={model.get_num_params():,}', flush=True)
elif hasattr(layer, 'core_dim'):
    print(f'e10_k256: dim={model.dim} depth={model.depth} core={layer.core_dim} mem={layer.mem_dim} params={model.get_num_params():,}', flush=True)
else:
    print(f'e10_k256: dim={model.dim} depth={model.depth} params={model.get_num_params():,}', flush=True)
...
elapsed = time.time() - start
tokens = step * batch_size * 512
print(f'DONE: e10_k256 | Steps={step} | Time={elapsed:.0f}s | Last100={np.mean(losses[-100:]):.4f} | {int(tokens/elapsed)} tok/s | PeakMem={peak_mem:.1f}GB', flush=True)
mm.close()
