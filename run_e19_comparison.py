#!/usr/bin/env python3
"""
E19 Comparison: 10-minute training time comparison.
Tests E1, Mamba2, minLSTM, E18-A (best E18), and all E19 variants.
...
from elman.models import LadderLM
name = "E19A"
model = LadderLM(vocab_size=256, dim=1280, depth=6, level='19a').cuda().bfloat16()
'''
    },
...
from elman.models import LadderLM
name = "E19B"
model = LadderLM(vocab_size=256, dim=1280, depth=6, level='19b').cuda().bfloat16()
'''
    },
    # E19-D: residual + z gate
    {
        "name": "E19D",
        "gpu": 6,
        "setup": '''
from elman.models import LadderLM
name = "E19D"
model = LadderLM(vocab_size=256, dim=1280, depth=6, level='19d').cuda().bfloat16()
'''
    },
    # E19-E: residual + Wx + h gate
    {
        "name": "E19E",
        "gpu": 7,
        "setup": '''
from elman.models import LadderLM
name = "E19E"
model = LadderLM(vocab_size=256, dim=1280, depth=6, level='19e').cuda().bfloat16()
'''
    },
]

OUT = "benchmark_results/e19_comparison"
os.makedirs(OUT, exist_ok=True)

print(f"E19 10-minute comparison: {TIME_LIMIT}s per model")
...
for name, _, log_path in procs:
    with open(log_path) as f:
        ...
        for line in content.split('\n'):
            if 'params' in line and name in line:
                params = line
        # Find final line
        for line in content.split('\n'):
            if 'FINAL' in line:
