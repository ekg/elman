#!/usr/bin/env python3
"""E10 sweep: 9 configs, ~50M params each, max batch size per model."""
import os, subprocess, time, sys
sys.path.insert(0, '/home/erikg/elman')

TIME_LIMIT = 600
OUT = "benchmark_results/e10_sweep"
    ...
    for dim in range(256, 2048, 32):
        try:
            ...
            if diff < best_diff:
                best_diff = diff
                best_dim = dim
            # Stop if we've gone past target and diff is increasing
...
# Find configs - search for right dim for each
configs = []

# E1 baseline (depth=6)
dim = find_dim_for_50m(1, 6, 1.0, 4, 0.5)
params = get_params(1, dim, 6, 1.0, 4, 0.5)
configs.append(("e1", 1, dim, 6, 1.0, 4, 0.5, params))
print(f"E1:       dim={dim}, depth=6, params={params:,}")

# E9 baseline (depth=3, expansion=2.0)
dim = find_dim_for_50m(9, 3, 2.0, 4, 0.5)
params = get_params(9, dim, 3, 2.0, 4, 0.5)
configs.append(("e9", 9, dim, 3, 2.0, 4, 0.5, params))
print(f"E9:       dim={dim}, depth=3, exp=2.0, params={params:,}")

# E10 with varying k (depth=6)
for k in [4, 8, 16, 32, 64, 128, 256]:
    dim = find_dim_for_50m(10, 6, 1.0, k, 0.5)
    params = get_params(10, dim, 6, 1.0, k, 0.5)
    configs.append((f"e10_k{k}", 10, dim, 6, 1.0, k, 0.5, params))
    print(f"E10 k={k:<3}: dim={dim}, depth=6, params={params:,}")

# Set batch sizes - conservative estimates based on model size
# More banks = more activation memory per sample
batch_sizes = {
    "e1": 256,
    "e9": 192,
    "e10_k4": 192,
        ...
        with open(f"{OUT}/{name}.log") as f:
            for line in f:
                if "DONE:" in line:
                    ...
                    results.append((name, n_banks, dim, params, loss, toks, mem))
    except: pass

print(f"{'Name':<12} {'k':<4} {'dim':<5} {'params':<12} {'Loss':<8} {'Tok/s':<10} {'Mem'}")
print("-" * 70)
for name, k, dim, params, loss, toks, mem in sorted(results, key=lambda x: x[4]):
    print(f"{name:<12} {k:<4} {dim:<5} {params:<12,} {loss:<8.4f} {toks:<10,} {mem:.1f}GB")
