"""
Level 2: Selective Elman - Gated recurrence + compete x silu output
...
Key question: Does output selectivity (compete x silu) help beyond gating?
...
class SelectiveElmanFunction(torch.autograd.Function):
    ...
    def forward(ctx, training, x, h0, W_x, W_h, W_delta, W_out, b, b_delta, n_groups):
        h, output, v, delta_cache, compete_cache = hasty_pytorch_lib.selective_elman_forward(
            training,
            x.contiguous(),
            h0.contiguous(),
            W_x.contiguous(),
    ...
    def backward(ctx, dh_out, d_output):
        ...
        dx, dW_x, dW_h, dW_delta, dW_out, db, db_delta = hasty_pytorch_lib.selective_elman_backward(
...
class SelectiveElman(nn.Module):
    ...
    Gated recurrence + compete x silu output selectivity.
...
if __name__ == "__main__":
    ...
    model = SelectiveElman(dim=512, expansion=2.0, n_groups=32).to(device).bfloat16()
