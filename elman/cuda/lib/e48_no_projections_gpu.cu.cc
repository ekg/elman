...
// Same recurrence as E42 but with dim×dim weight matrix (no expansion).
//
// Architecture:
...
//
// Key optimization: Batch W @ x for ALL timesteps, then
...
__global__ void E48FusedLinearGateKernel_BF16(
...
template<typename T>
__global__ void E48FusedLinearGateKernel(
    const int batch_size,
    const int dim,
...
// =============================================================================
// Backward Kernels (E48: Linear recurrence - no tanh derivative!)
// =============================================================================

// E48: Backward through LINEAR recurrence - gradient flows directly (derivative is 1)
__global__ void E48LinearBackwardKernel_BF16(
    const int batch_size,
    const int dim,
    ...
    if (idx < total) {
        ...
        // E48: LINEAR backward - gradient flows directly (derivative is 1)
...
template<typename T>
__global__ void E48LinearBackwardKernel(
    const int batch_size,
    const int dim,
...
__global__ void E48SelfGateBackward_BF16(
    const int batch_size,
    const int dim,
...
template<typename T>
__global__ void E48SelfGateBackward(
    const int batch_size,
    const int dim,
...
__global__ void E48VectorAddInplace_BF16(
    const int n,
...
template<typename T>
__global__ void E48VectorAddInplace(const int n, T* __restrict__ a, const T* __restrict__ b) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
...
__global__ void E48VectorAdd_BF16(
    const int n,
...
template<typename T>
__global__ void E48CopyFloatToT(const int n, const float* __restrict__ src, T* __restrict__ dst) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
...
// =============================================================================
// E48 No Projections Forward - BF16 Specialization
// =============================================================================

template<>
E48NoProjectionsForward<__nv_bfloat16>::E48NoProjectionsForward(
    bool training,
    int batch_size,
    int dim,
...
template<>
void E48NoProjectionsForward<__nv_bfloat16>::Run(
    int steps,
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        // E48: Fused LINEAR + self-gate (NO TANH!)
        E48FusedLinearGateKernel_BF16<<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, Wx_t, tmp_Wh, b, h_t, out_t, v_t);
    }
}

// =============================================================================
// E48 No Projections Backward - BF16 Specialization
// =============================================================================

template<>
E48NoProjectionsBackward<__nv_bfloat16>::E48NoProjectionsBackward(
    int batch_size,
    int dim,
    const cublasHandle_t& blas_handle,
    const cudaStream_t& stream)
...
template<>
void E48NoProjectionsBackward<__nv_bfloat16>::Run(
    int steps,
    ...
    for (int t = steps - 1; t >= 0; --t) {
        ...
        E48SelfGateBackward_BF16<<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, h_t, d_out_t, dh);

        // Add recurrent gradient
        E48VectorAddInplace_BF16<<<num_blocks, block_size, 0, stream_>>>(BD, dh, dh_recurrent);

        // E48: Backward through LINEAR recurrence (NO tanh derivative!)
        E48LinearBackwardKernel_BF16<<<num_blocks, block_size, 0, stream_>>>(
    ...
    E48VectorAdd_BF16<<<add_blocks, block_size, 0, stream_>>>(
    ...
    E48CopyFloatToT<__nv_bfloat16><<<(dim_ + 255) / 256, 256, 0, stream_>>>(dim_, db_float, db);
}
...
template<typename T>
E48NoProjectionsForward<T>::E48NoProjectionsForward(
    bool training,
    int batch_size,
    int dim,
    const cublasHandle_t& blas_handle,
...
template<typename T>
void E48NoProjectionsForward<T>::Run(
    int steps,
    const T* W,
    const T* b,
    const T* x,
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        // E48: Fused LINEAR + self-gate kernel (NO TANH!)
        E48FusedLinearGateKernel<T><<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, Wx_t, tmp_Wh, b, h_t, out_t, v_t);
    }
}

template<typename T>
E48NoProjectionsBackward<T>::E48NoProjectionsBackward(
    int batch_size,
    int dim,
    const cublasHandle_t& blas_handle,
    const cudaStream_t& stream)
...
template<typename T>
void E48NoProjectionsBackward<T>::Run(
    int steps,
    const T* W,
    const T* x,
    const T* h,
    ...
    for (int t = steps - 1; t >= 0; --t) {
        ...
        E48SelfGateBackward<T><<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, h_t, d_out_t, dh);

        E48VectorAddInplace<T><<<num_blocks, block_size, 0, stream_>>>(BD, dh, dh_recurrent);

        // E48: LINEAR backward (NO tanh derivative)
        E48LinearBackwardKernel<T><<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, v_t, dh, nullptr, dv_t, db_float);
    ...
    const int total_elements = steps * BD;
    const int add_blocks = (total_elements + block_size - 1) / block_size;
    E48VectorAdd<T><<<add_blocks, block_size, 0, stream_>>>(
        total_elements, x, h, x_plus_h);
    ...
    E48CopyFloatToT<T><<<(dim_ + 255) / 256, 256, 0, stream_>>>(dim_, db_float, db);
}

// Explicit template instantiations
template struct E48NoProjectionsForward<__half>;
template struct E48NoProjectionsForward<float>;
template struct E48NoProjectionsForward<double>;

template struct E48NoProjectionsBackward<__half>;
template struct E48NoProjectionsBackward<float>;
template struct E48NoProjectionsBackward<double>;
