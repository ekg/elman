...
// - No W_x matrix (like E38)
// - No pre-silu (input goes directly to recurrence)
//
// Architecture:
// x_proj = in_proj(x)                     # No silu! (pre-computed before kernel)
// h_t = tanh(x_t + W_h @ h_{t-1} + b)     # x goes directly, no W_x
...
// =============================================================================
// E40 Forward Kernels - NO W_x, just x + Rh + b
// =============================================================================

// BF16-optimized: Fused x + Rh + bias + tanh + self-gate
// Key difference from E33: No Wx term, x goes directly
__global__ void E40FusedTanhGateKernel_BF16(
    const int batch_size,
    const int dim,
    const __nv_bfloat16* __restrict__ x_t,   // [B, dim] - raw input (NOT pre-activated)
    const __nv_bfloat16* __restrict__ Rh,    // [B, dim] - W_h @ h_prev
    const __nv_bfloat16* __restrict__ b,     // [dim]
    __nv_bfloat16* __restrict__ h_out,       // [B, dim]
...
template<typename T>
__global__ void E40FusedTanhGateKernel(
    const int batch_size,
    const int dim,
...
// =============================================================================
// E40 Backward Kernels
// =============================================================================

// BF16-optimized backward through tanh
__global__ void E40TanhBackwardKernel_BF16(
    const int batch_size,
    const int dim,
...
// Generic version
template<typename T>
__global__ void E40TanhBackwardKernel(
    const int batch_size,
    const int dim,
...
__global__ void E40SelfGateBackward_BF16(
    const int batch_size,
    const int dim,
...
// Generic version
template<typename T>
__global__ void E40SelfGateBackward(
    const int batch_size,
    const int dim,
...
// =============================================================================
// E40 No Pre-SiLU Forward - BF16 Specialization
// =============================================================================

template<>
E40NoPresiluForward<__nv_bfloat16>::E40NoPresiluForward(
    bool training,
    int batch_size,
    int dim,
...
template<>
void E40NoPresiluForward<__nv_bfloat16>::Run(
    int steps,
    ...
    // E40: No Wx precompute! Just need tmp_Rh
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        // E40 FUSED: h_t = tanh(x_t + tmp_Rh + b), output = h_t * silu(h_t)
        E40FusedTanhGateKernel_BF16<<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, x_t, tmp_Rh, b, h_t, out_t, v_t);
    }
}

// =============================================================================
// E40 No Pre-SiLU Backward - BF16 Specialization
// =============================================================================

template<>
E40NoPresiluBackward<__nv_bfloat16>::E40NoPresiluBackward(
    int batch_size,
    int dim,
    const cublasHandle_t& blas_handle,
    const cudaStream_t& stream)
...
template<>
void E40NoPresiluBackward<__nv_bfloat16>::Run(
    int steps,
    const __nv_bfloat16* W_h,
    const __nv_bfloat16* x,
    ...
    for (int t = steps - 1; t >= 0; --t) {
        ...
        E40SelfGateBackward_BF16<<<num_blocks, block_size, 0, stream_>>>(
        ...
        E40TanhBackwardKernel_BF16<<<num_blocks, block_size, 0, stream_>>>(
...
template<typename T>
E40NoPresiluForward<T>::E40NoPresiluForward(
    bool training,
    int batch_size,
    int dim,
    const cublasHandle_t& blas_handle,
...
template<typename T>
void E40NoPresiluForward<T>::Run(
    int steps,
    const T* W_h,
    const T* b,
    const T* x,
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        E40FusedTanhGateKernel<T><<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, x_t, tmp_Rh, b, h_t, out_t, v_t);
    }
}

template<typename T>
E40NoPresiluBackward<T>::E40NoPresiluBackward(
    int batch_size,
    int dim,
    const cublasHandle_t& blas_handle,
    const cudaStream_t& stream)
...
template<typename T>
void E40NoPresiluBackward<T>::Run(
    int steps,
    const T* W_h,
    const T* x,
    const T* h,
    ...
    for (int t = steps - 1; t >= 0; --t) {
        ...
        E40SelfGateBackward<T><<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, h_t, d_out_t, dh);

        VectorAddInplace<T><<<num_blocks, block_size, 0, stream_>>>(BD, dh, dh_recurrent);

        E40TanhBackwardKernel<T><<<num_blocks, block_size, 0, stream_>>>(
            batch_size_, dim_, v_t, dh, nullptr, dv_t, db_float);
...
template struct E40NoPresiluForward<__half>;
template struct E40NoPresiluForward<float>;
template struct E40NoPresiluForward<double>;

template struct E40NoPresiluBackward<__half>;
template struct E40NoPresiluBackward<float>;
template struct E40NoPresiluBackward<double>;
