...
// output_t = h_t * silu(x_t + b_gate)  -- x-only input-dependent gating
//
...
__global__ void TanhBackwardKernel(
    ...
    float* __restrict__ db) {          // [dim] gradient w.r.t. bias
...
// =============================================================================
// X-Only Selective Output: output = h * silu(gate_proj + b_gate)
// where gate_proj = x @ W_gate (pre-computed via GEMM)
// Note: h is NOT included in the silu argument (unlike h+x gating)
// =============================================================================
...
__global__ void XGatedOutputForward(
    ...
    const T* __restrict__ gate_proj,    // [B, dim] pre-computed x @ W_gate
    const T* __restrict__ b_gate,       // [dim] learned gate bias
    T* __restrict__ output,             // [B, dim]
    T* __restrict__ gate_cache) {       // [B, dim] pre-silu value for backward
    ...
    if (idx < total) {
        ...
        // X-only gating: silu(gate_proj + b_gate)
        ...
        // Cache pre-silu value for backward (gate_raw)
...
__global__ void XGatedOutputBackward(
    ...
    if (idx < total) {
        ...
        // Recompute silu from cached gate_raw
        float sigmoid_val = 1.0f / (1.0f + expf(-gate_raw));
        float silu_val = gate_raw * sigmoid_val;
...
// =============================================================================
// X-Gated Elman Forward
// =============================================================================

template<typename T>
XGatedElmanForward<T>::XGatedElmanForward(
    bool training,
    int batch_size,
    int dim,
...
void XGatedElmanForward<T>::Run(
    ...
    T* tmp_Wx;
    T* tmp_Rh;
    T* gate_proj;  // Pre-computed x @ W_gate for gating
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        // output = h * silu(gate_proj + b_gate) -- X-ONLY GATING with W_gate
...
// =============================================================================
// X-Gated Elman Backward
// =============================================================================

template<typename T>
XGatedElmanBackward<T>::XGatedElmanBackward(
    int batch_size,
    int dim,
    const cublasHandle_t& blas_handle,
    const cudaStream_t& stream)
...
template<typename T>
void XGatedElmanBackward<T>::Run(
    int steps,
    const T* W_x,
    const T* W_h,
    const T* W_gate,
    ...
    // BPTT loop with x-only selective output (using W_gate)
    for (int t = steps - 1; t >= 0; --t) {
        ...
        // Backward through x-only selective output: output = h * silu(gate_proj + b_gate)
        // Uses cached gate_raw = gate_proj + b_gate from forward
        ...
        TanhBackwardKernel<T><<<num_blocks, block_size, 0, stream>>>(
    ...
    // Initialize dx to zero before accumulating
...
template struct XGatedElmanForward<__half>;
template struct XGatedElmanForward<__nv_bfloat16>;
template struct XGatedElmanForward<float>;
template struct XGatedElmanForward<double>;

template struct XGatedElmanBackward<__half>;
template struct XGatedElmanBackward<__nv_bfloat16>;
template struct XGatedElmanBackward<float>;
template struct XGatedElmanBackward<double>;
