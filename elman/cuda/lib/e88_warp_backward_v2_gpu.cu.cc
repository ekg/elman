/**
 * E88 Warp Backward V2 - Uses segment_cache for k, v in backward phase
 *
 * Key difference from warp_backward_simple:
 * - In backward phase, reads k, v from segment_cache (like fused backward)
...
#define E88_WARP_BACKWARD_V2_CHUNK_SIZE 16

namespace elman {

__device__ __forceinline__ float e88_v2_tanh(float x) {
    return tanhf(x);
}

__device__ __forceinline__ float e88_v2_silu(float x) {
    return x / (1.0f + expf(-x));
}

__device__ __forceinline__ float e88_v2_sigmoid(float x) {
    return 1.0f / (1.0f + expf(-x));
}

template<int N_STATE, int HEAD_V_DIM, int CHUNK_SIZE>
__global__ void E88WarpBackwardV2Kernel_BF16(
    int T,
    int B,
    int H,
    ...
    float* q = v + HEAD_V_DIM;                  // Single timestep q buffer
    float* k_chunk = q + N_STATE;               // For forward replay prefetch
    float* v_chunk = k_chunk + CHUNK_SIZE * N_STATE;
    float* decay_chunk = v_chunk + CHUNK_SIZE * HEAD_V_DIM;
...
void dispatch_e88_warp_backward_v2(
    ...
    #define DISPATCH_V2(NS, HV) \
        do { \
            size_t shmem_size = (NS * HV) * 4 + \
                                NS + HV + NS + \
                                (CHUNK_SIZE * NS) + (CHUNK_SIZE * HV) + \
                                CHUNK_SIZE + (has_gate ? CHUNK_SIZE * HV : 0) + \
                                HV + HV + NS + NS + HV + 8; \
            shmem_size *= sizeof(float); \
            E88WarpBackwardV2Kernel_BF16<NS, HV, CHUNK_SIZE><<<num_blocks, threads_per_block, shmem_size, stream>>>( \
    ...
    if (n_state == 32 && head_v_dim == 32) {
        ...
        printf("E88 Warp Backward V2: unsupported n_state=%d head_v_dim=%d\n", n_state, head_v_dim);
    }
