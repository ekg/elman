...
// output_t = h_t * silu(W_gate @ x_t + b_gate)  -- learned gate projection
...
void DiagonalSelectiveElmanForward<T>::Run(
    ...
    // Reduces from 3*T GEMMs to 3 big GEMMs (much more efficient)
    // =========================================================================
    T *all_wx_x, *all_delta_raw, *all_gate_proj;
    ...
    // Pre-compute W_gate @ x for ALL timesteps in one GEMM (learned gate projection)
    blas<T>::gemm(blas_handle_, CUBLAS_OP_T, CUBLAS_OP_N,
        dim_, steps * batch_size_, dim_, &alpha, W_gate, dim_, x, dim_, &beta_zero, all_gate_proj, dim_);

    // Per-timestep: diagonal gated update + learned gate projection output
...
void DiagonalSelectiveElmanBackward<T>::Run(
    ...
    for (int t = steps - 1; t >= 0; --t) {
        ...
        // dx = W_x @ dv + W_delta @ d_delta_raw + W_gate @ d_gate_proj
        ...
        blas<T>::gemm(blas_handle_, CUBLAS_OP_N, CUBLAS_OP_N,
            dim_, batch_size_, dim_, &alpha, W_gate, dim_, d_gate_proj, dim_, &alpha, dx_t, dim_);

        // dh_prev from DiagonalSelectiveGatedBackward is the correct gradient to propagate
        // (dh_selective already had dh_recurrent added before the call)
        // Copy to dh_recurrent for next iteration
        cudaMemcpyAsync(dh_recurrent, dh_prev, BD * sizeof(T), cudaMemcpyDeviceToDevice, stream_);

        // Weight gradients: dW = d_proj @ x.T
