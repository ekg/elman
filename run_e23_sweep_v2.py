#!/usr/bin/env python3
"""
E23 Configuration Sweep v2: Only CUDA-supported configs

CUDA kernel supports: (n_slots, dim) combinations
- n_slots=8: dim ∈ {256, 512, 768, 1024}
- n_slots=16: dim ∈ {256, 512}
- n_slots=32: dim ∈ {512}
- n_slots=64: dim ∈ {256, 512, 768, 1024}

Use smaller batch size for high-N configs to avoid OOM.
"""
...
# Sweep configurations: (dim, n_slots, depth, batch_size, name)
# Target ~50M params each
# Smaller batch for high-N configs to avoid OOM
CONFIGS = [
    # Smaller D, larger N - the hypothesis (need small batch)
    (256, 64, 96, 32, "d256_n64"),   # ~50M, smaller D, large N
    (512, 64, 32, 32, "d512_n64"),   # ~50M, mid D, large N
    (768, 64, 14, 32, "d768_n64"),   # ~50M, larger D, large N
    (1024, 64, 8, 16, "d1024_n64"),  # ~50M, big D, large N (smallest batch)

    # Baseline configs (can use larger batch)
    (512, 32, 32, 64, "d512_n32"),   # ~50M, baseline
    (512, 16, 32, 64, "d512_n16"),   # ~50M, smaller tape
    (512, 8, 32, 64, "d512_n8"),     # ~50M, minimal tape
    (768, 8, 14, 64, "d768_n8"),     # ~50M, like E1 + tiny tape
]

DATA_PATH = "/home/erikg/elman/data/pile.txt"
OUTPUT_DIR = Path("/home/erikg/elman/benchmark_results/e23_sweep_v2")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Create the worker script template
WORKER_SCRIPT = """
...
print(f'E23 D={{dim}} N={{n_slots}} depth={{depth}} batch={{batch_size}}: params={{n_params:,}}', flush=True)
...
def run_config(gpu_id, dim, n_slots, depth, batch_size, name):
    ...
    with open(log_file, "w") as f:
        f.write(f"Config: D={dim}, N={n_slots}, depth={depth}, batch={batch_size}\n")
        f.write("=" * 60 + "\n\n")
        f.flush()
...
def main():
    print("=" * 70)
    print("E23 Configuration Sweep v2 (CUDA-supported configs only)")
    print("=" * 70)
    ...
    processes = []
    for gpu_id, (dim, n_slots, depth, batch_size, name) in enumerate(CONFIGS):
        if gpu_id >= 8:
            print(f"Warning: Only 8 GPUs available, skipping {name}")
            continue
        proc, name, log_file = run_config(gpu_id, dim, n_slots, depth, batch_size, name)
        processes.append((proc, name, log_file, dim, n_slots, depth, batch_size))

    print(f"\nLaunched {len(processes)} experiments. Waiting for completion...")
    print("Check progress with: tail -f benchmark_results/e23_sweep_v2/*.log")
    print()

    # Wait for all to complete
    start_time = time.time()
    while True:
        running = sum(1 for p, _, _, _, _, _, _ in processes if p.poll() is None)
        if running == 0:
            break
    ...
    print("=" * 80)
    print(f"{'Config':<12} | {'D':>5} | {'N':>3} | {'Depth':>5} | {'Batch':>5} | {'Params':>8} | {'Loss':>8} | {'Throughput':>12}")
    print("-" * 80)

    results = []
    for proc, name, log_file, dim, n_slots, depth, batch_size in processes:
        ...
        print(f"{name:<12} | {dim:>5} | {n_slots:>3} | {depth:>5} | {batch_size:>5} | {params_str:>8} | {loss_str:>8} | {tp_str:>12}")
    ...
    for i, r in enumerate(sorted(valid_results, key=lambda x: x["loss"]), 1):
        ...
        print(f"{i}. {r['name']:<12} loss={r['loss']:.4f}  throughput={tp_str} tok/s  (D={r['dim']}, N={r['n_slots']})")
    ...
    for i, r in enumerate(sorted(valid_results, key=lambda x: -x["throughput"] if x["throughput"] else 0), 1):
        tp_str = f"{r['throughput']/1000:.1f}K" if r["throughput"] else "N/A"
        print(f"{i}. {r['name']:<12} throughput={tp_str} tok/s  loss={r['loss']:.4f}  (D={r['dim']}, N={r['n_slots']})")
