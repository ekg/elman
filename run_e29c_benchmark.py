#!/usr/bin/env python3
"""
E29c Benchmark: Compare E29c (SSM-style diagonal gating) vs E29a vs E1 vs Mamba2

E29c uses element-wise gating (4*D params) instead of:
  - E29a: additive gate (0 extra params)
  - E29b: matrix gate (3*D² params)

Run 4-8 models in parallel on GPUs.
"""
...
OUTPUT_DIR = Path("/home/erikg/elman/benchmark_results/e29c_comparison")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Common parameters
DIM = 1024  # E29 CUDA kernels max dim is 1024 (for hybrid template approach)
DEPTH = 6
BATCH_SIZE = 64
SEQ_LEN = 512
...
# Model configurations (model_type, name, extra_args)
CONFIGS = [
    ("mamba2", "mamba2_d1024_depth6", {"dim": 1024, "depth": 6}),
    ("e1", "e1_d1024_depth6", {"dim": 1024, "depth": 6}),
    ("e29a", "e29a_d1024_n8", {"dim": 1024, "depth": 6, "n_slots": 8}),
    ("e29c", "e29c_d1024_n8", {"dim": 1024, "depth": 6, "n_slots": 8}),
]
...
if model_type == "mamba2":
    ...
    # Use LadderLM for proper fused add+norm and residual pattern
    from elman.models import LadderLM
    model = LadderLM(vocab_size=256, dim=dim, depth=depth, level=1, expansion=1.0).cuda().bfloat16()

elif model_type == "e29a":
    from elman.models.e29_selective import E29aSelectiveElmanCell
    from mamba_ssm.ops.triton.layer_norm import RMSNorm, rms_norm_fn

    class E29aLM(nn.Module):
        \"\"\"E29a LM with proper fused add+norm (like LadderLM).\"\"\"
        def __init__(self, vocab_size, dim, depth, n_slots):
            super().__init__()
            self.vocab_size = vocab_size
            self.embed = nn.Embedding(vocab_size, dim)
            self.layers = nn.ModuleList([E29aSelectiveElmanCell(dim, n_slots=n_slots) for _ in range(depth)])
        ...
        def forward(self, x, return_loss=False):
            ...
            for ln, layer in zip(self.layer_norms, self.layers):
                x, residual = rms_norm_fn(x, ln.weight, None, residual=residual,
                                          prenorm=True, residual_in_fp32=True, eps=ln.eps)
                x, _, _ = layer(x, use_cuda=True)
            x = rms_norm_fn(x, self.norm.weight, None, residual=residual,
                            prenorm=False, residual_in_fp32=True, eps=self.norm.eps)
            logits = self.lm_head(x)
            if return_loss:
                return F.cross_entropy(logits.view(-1, self.vocab_size), targets.view(-1))
            return logits

    model = E29aLM(256, dim, depth, n_slots).cuda().bfloat16()

elif model_type == "e29c":
    from elman.models.e29c_diagonal import E29cDiagonalElmanCell
    from mamba_ssm.ops.triton.layer_norm import RMSNorm, rms_norm_fn

    class E29cLM(nn.Module):
        \"\"\"E29c LM with proper fused add+norm (like LadderLM).\"\"\"
        def __init__(self, vocab_size, dim, depth, n_slots):
            super().__init__()
            self.vocab_size = vocab_size
            self.embed = nn.Embedding(vocab_size, dim)
            self.layers = nn.ModuleList([E29cDiagonalElmanCell(dim, n_slots=n_slots) for _ in range(depth)])
            self.layer_norms = nn.ModuleList([RMSNorm(dim) for _ in range(depth)])
            self.norm = RMSNorm(dim)
            self.lm_head = nn.Linear(dim, vocab_size, bias=False)
            self.lm_head.weight = self.embed.weight
    ...
    model = E29cLM(256, dim, depth, n_slots).cuda().bfloat16()
...
def main():
    print("=" * 70)
    print("E29c Benchmark: SSM-Style Diagonal Gating")
    print("=" * 70)
    ...
    # Analysis
    print("\n" + "=" * 70)
    print("Analysis: E29c vs E29a (should be similar throughput, E29c has 4*D more params)")
    print("=" * 70)
    ...
    e29c = next((r for r in results if r["model_type"] == "e29c"), None)

    if e1 and e1["loss"]:
        print(f"\nE1 (baseline): loss={e1['loss']:.4f}, {e1['throughput']/1000:.1f}K tok/s")

        for name, model in [("Mamba2", mamba2), ("E29a", e29a), ("E29c", e29c)]:
    ...
    if e29a and e29c and e29a["loss"] and e29c["loss"]:
        print(f"\nE29c vs E29a direct comparison:")
        loss_diff = e29c["loss"] - e29a["loss"]
        tp_ratio = e29c["throughput"] / e29a["throughput"] if e29a["throughput"] else 0
        print(f"  Loss: {e29c['loss']:.4f} vs {e29a['loss']:.4f} (diff: {loss_diff:+.4f})")
        print(f"  Throughput: {e29c['throughput']/1000:.1f}K vs {e29a['throughput']/1000:.1f}K (ratio: {tp_ratio:.2f}x)")
        print(f"  Extra params: ~{4*1024/1000:.1f}K (4*D diagonal gate weights)")
